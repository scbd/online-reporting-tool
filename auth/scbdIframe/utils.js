...
async function getScbdIframeToken () {
  ...
  function receivePostMessage (event) {
    const authConf = useAuthConf();
    if (!event.data || authConf.accountsHostUrl !== event.origin) { return; }
    ...
    if (!['authenticationTokenUpdated', 'authenticationToken'].includes(type)) { return; }// throw new Error('unsupported authentication message type');
  ...
  function resolveToken (ms = 300) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      const accountsIframe = getScbdIframe();
...
function getScbdIframe () {
  ...
  if (iFrames) {
    ...
    if (authConf.accountsHostUrl === origin) { return iFrames; }
  }
}
...
function createScbdIframe (onloadCallback) {
  ...
  const sc = document.createElement('iframe');

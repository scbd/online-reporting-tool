...
export const useRealmConfStore = defineStore('realmConf', {
    ...
    schemaRoles(state) {
      return (schema, roleName)=>{
        ...
        if(roleName){
            ...
            roles = _.union(schemaDetails.publishingAuthorities||this.fallbackRoles(state)(schemaDetails.type, 'publishingAuthorities'), 
                            schemaDetails.nationalAuthorizedUser||this.fallbackRoles(state)(schemaDetails.type, 'nationalAuthorizedUser'));
  ...
  actions:{
    async loadRealmConf(){
        console.log('h', this, this.$nuxt.$api)
        if(!this.realmConf?.realm){
          const realmConf = await this.$nuxt.$api.realmConfiguration.getRealmConfigurationByHost(this.$nuxt.$config.REALM_CONF_HOST);
